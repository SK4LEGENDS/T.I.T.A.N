...
export default function TimetableView({ currentLab, generatedData }) {
  ...
  useEffect(() => {
    ...
    if (generatedData && generatedData.timetables) {
      ...
      // Try to load full timeslots from cached CSV so that ALL periods (including empty ones) are shown!
      const cachedCsv = localStorage.getItem('cached_timeSlotsCsv');
      let parsedTimeSlots = [];
      if (cachedCsv) {
          ...
          if (parts.length > 0) {
            ...
            if (timeVal) {
              if (cleanP.includes('break') || cleanT.includes('break')) {
                parsedTimeSlots.push(cleanT.includes('break') ? timeVal : `BREAK (${timeVal})`);
              } else if (cleanP.includes('lunch') || cleanT.includes('lunch')) {
                parsedTimeSlots.push(cleanT.includes('lunch') ? timeVal : `LUNCH (${timeVal})`);
              } else {
                parsedTimeSlots.push(timeVal);
              }
            }
          }
        }
      ...
      // Ensure we display all timeslots from the CSV, plus any timeslots actually generated by the AI
      // CRITICAL: Normalize and deduplicate to prevent broken quote-split columns like '"BREAK' and '"LUNCH'
      const rawCombined = [...parsedTimeSlots, ...Array.from(uniqueTimes)];
      const normalizedSlots = [];
      const seenBreak = new Set();
      const seenLunch = new Set();
      const seenAcademic = new Set();

      for (const slot of rawCombined) {
        // Strip any leftover quote characters
        const clean = slot.replace(/"/g, '').replace(/'/g, '').trim();
        if (!clean) continue;

        const lower = clean.toLowerCase();

        // Skip pure malformed fragments like just "Break" or "Lunch" without time info
        if (lower === 'break' || lower === 'lunch') continue;

        if (lower.includes('break')) {
          // Only keep one break column — prefer the longest/most descriptive version
          if (seenBreak.size === 0) {
            seenBreak.add(clean);
            normalizedSlots.push(clean);
          } else {
            // Replace with longer version if this one is more descriptive
            const existing = Array.from(seenBreak)[0];
            if (clean.length > existing.length) {
              const idx = normalizedSlots.indexOf(existing);
              if (idx !== -1) normalizedSlots[idx] = clean;
              seenBreak.clear();
              seenBreak.add(clean);
            }
          }
        } else if (lower.includes('lunch')) {
          // Only keep one lunch column — prefer the longest/most descriptive version
          if (seenLunch.size === 0) {
            seenLunch.add(clean);
            normalizedSlots.push(clean);
          } else {
            const existing = Array.from(seenLunch)[0];
            if (clean.length > existing.length) {
              const idx = normalizedSlots.indexOf(existing);
              if (idx !== -1) normalizedSlots[idx] = clean;
              seenLunch.clear();
          ...
          // Regular academic slot — deduplicate by normalized time string
          const normKey = lower.replace(/\s+/g, '');
          if (!seenAcademic.has(normKey)) {
            seenAcademic.add(normKey);
            normalizedSlots.push(clean);
          }
        }
      }

      const combinedTimes = new Set(normalizedSlots);
      
      if (combinedTimes.size > 0) {
        setTimeSlots(Array.from(combinedTimes).sort((a, b) => {
           const parseTime = (t) => {
             const cleanT = t.toLowerCase();
             if (cleanT.includes('break')) return 561; // Sorts perfectly between 8:40 AM and 9:30 AM (around 9:20 AM)
             if (cleanT.includes('lunch')) return 691; // Sorts perfectly between 10:50 AM and 12:00 PM (around 11:30 AM)
